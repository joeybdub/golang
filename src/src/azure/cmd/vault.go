...
func init() {
	...
	vaultCmd.Flags().BoolP("subscription", "s", false, "The azure subscription")
...
func secret(args []string) {
	stringArray := args
	subscription := strings.Join(stringArray, " ")
	...
	secret, err := client.Logical().Read("secret/accounts/" + subscription + "/azure")
	if err != nil {
		fmt.Println(err)
		return
	}
